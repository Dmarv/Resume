...
numsvl1 = 0;
numsvm1 = 0;
total_lamb = zeros(1, 2);
total_mu = zeros(1, 2);
u = u';
v = v';
for i=1:p
    if lamb(i) > 10^(-10)
        total_lamb = total_lamb + u(i, 1:2);
        numsvl1 = numsvl1 + 1;
    end
end
for i=1:q 
    if mu(i) > 10^(-10)
        total_mu = total_mu + v(i, 1:2);
        numsvm1 = numsvm1 + 1;
    end
end
average_lamb = total_lamb/numsvl1;
average_mu = total_mu/numsvm1;
b = w' * ((average_lamb + average_mu)/2)';
